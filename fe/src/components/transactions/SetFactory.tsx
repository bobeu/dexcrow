...
interface SetFactoryProps {
  newFactory: string;
  onSuccess?: () => void;
  onError?: (error: string) => void;
}

const SetFactory: React.FC<SetFactoryProps> = ({
  newFactory,
  onSuccess,
  onError,
}) => {
  ...
  return (
    <>
      <Button
        ...
        disabled={isPending || isConfirming || !newFactory}
        variant="primary"
        className="w-full"
      >
        {isPending || isConfirming ? 'Setting Factory...' : 'Set Factory'}
      </Button>
      ...
      <TransactionModal
        ...
        title="Set Factory"
        description={`Setting factory to: ${newFactory}`}
